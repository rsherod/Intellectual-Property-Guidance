    ...
    if 'db' in globals() and db:
        try:
            message_data = {
                ...
                "uploaded_policies": list(st.session_state.uploaded_policies.keys()) if st.session_state.uploaded_policies else []
            }
            doc_ref = db.collection("ip_licensing_chats").add(message_data)
        except Exception as e:
            st.session_state.debug.append(f"Firestore Save Error: {e}")
...
def initialize_firebase():
    try:
        ...
        db = firestore.client()
        st.session_state.debug.append("Firestore client obtained.")
        return db

    except Exception as e:
        st.error(f"Error initializing Firebase: {e}. Please check your secrets configuration.")
        st.session_state.debug.append(f"Firebase Init Error: {e}")
        return None
